...
export class RPStatsResolver {
  constructor(private readonly rpStatsRepo: RPStatsRepository, private readonly gangRepo: GangRepository, private readonly gangRankRepo: GangRankRepository, private readonly corporationRepo: CorporationRepository, private readonly corporationRankRepo: CorporationRankRepository) { }
  ...
  async gangRank(@Parent() { gangID, gangRankID }: RPStatsEntity): Promise<GangRankModel> {
    const matchingGangRank = await this.gangRankRepo.findOneOrFail({ gangID, gangRankID });
    return GangRankModel.fromEntity(matchingGangRank);
  }

  @ResolveField(() => CorporationModel, { nullable: true })
  async corporation(@Parent() { corporationID }: RPStatsEntity): Promise<CorporationModel> {
    const matchingCorp = await this.corporationRepo.findOneOrFail({ id: corporationID });
    return CorporationModel.fromEntity(matchingCorp);
  }

  @ResolveField(() => CorporationRankModel, { nullable: true })
  async corporationRank(@Parent() { corporationID, corporationRankID }: RPStatsEntity): Promise<CorporationRankModel> {
    const matchingCorpRank = await this.corporationRankRepo.findOneOrFail({ corporationID, corporationRankID });
