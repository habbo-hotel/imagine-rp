...
export class PhotoResolver {
  ...
  @Query(() => PhotoModel)
  async photo(@Args('filter') filter: PhotoFilterOneInput): Promise<PhotoEntity> {
    return this.photoDataloaderService.loadById(filter.id);
  }

  @Query(() => [PhotoModel])
  photos(@Args('filter', { type: () => PhotoFilterManyInput, nullable: true }) filter: PhotoFilterManyInput): Promise<PhotoEntity[]> {
