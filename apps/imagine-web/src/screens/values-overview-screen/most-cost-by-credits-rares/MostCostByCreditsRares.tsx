...
import { usefurniturePurchaseLogsOverviewTopCredits } from '@imagine-cms/client';
...
export function MostCostByCreditsRares() {
  const [page, setPage] = useState(0);
  const fetchMostCostByCredits = usefurniturePurchaseLogsOverviewTopCredits();

  const cardHeader = useMemo(() => (
    <div style={{ display: 'flex', flex: 1, justifyContent: 'space-between' }}>
      <div>Most credits</div>
      {page > 0 && <small>Page {page + 1}</small>}
    </div>
  ), [page]);

  const canGoUp = (fetchMostCostByCredits?.data?.length ?? 0) >= FURNITURE_PAGE_SIZE
  ...
  useEffect(() => {
    fetchMostCostByCredits.fetch({ skip: page * FURNITURE_PAGE_SIZE, limit: FURNITURE_PAGE_SIZE });
  }, [page]);
      ...
      <GridLarge>
        {canGoDown ?
          <ButtonNoBorder onClick={goBackOnePage}>
            <i className={fetchMostCostByCredits.loading ? 'fa fa-spinner fa-spin' : 'fa fa-arrow-left'} />
          </ButtonNoBorder>
          : <div />}
        {
          canGoUp && (
            <ButtonNoBorder onClick={goUpOnePage}>
              <i className={fetchMostCostByCredits.loading ? 'fa fa-spinner fa-spin' : 'fa fa-arrow-right'} />
            </ButtonNoBorder>
          )
        }
      </GridLarge>
